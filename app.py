...
def home():
    if request.method == 'GET':
        symptoms = request.args.getlist('symptomsList[]')
        if not symptoms:  # Kiểm tra xem symptoms có trống không
            return jsonify({'error': 'Symptoms list is empty'})
        else:
            print(symptoms)
            predicted_disease = get_predicted_value(symptoms)

            return jsonify({'diseases': predicted_disease})
